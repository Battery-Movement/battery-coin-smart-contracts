 ...
 *
 * BatteryCoinPresaleV2 - Modified version with ability to advance rounds without selling out
*/
...
contract BatteryCoinPresaleV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    event RoundForceAdvanced(uint256 indexed fromRound, uint256 indexed toRound, uint256 timestamp);
    event Reserved(
    ...
    /**
     * @dev Force advance to a specific round without requiring the previous rounds to be sold out
     * @param _roundNo The round number to advance to
     */
    function forceAdvanceToRound(uint256 _roundNo) external onlyOwners {
        require(_roundNo < presaleRoundCount, "Invalid round number");
        
        // Find current active round
        uint256 currentRoundNo;
        for (uint256 i = 0; i < presaleRoundCount; i++) {
            if (presaleRoundList[i].isStarted && !presaleRoundList[i].isFinished) {
                currentRoundNo = i;
                break;
            }
        }
        
        // Mark previous rounds as finished
        for (uint256 i = 0; i < _roundNo; i++) {
        ...
        PresaleRound storage newRound = presaleRoundList[_roundNo];
        newRound.startDate = block.timestamp;
        newRound.isStarted = true;
        newRound.isFinished = false;
        
        emit RoundSet(_roundNo, block.timestamp);
        emit RoundForceAdvanced(currentRoundNo, _roundNo, block.timestamp);
    }
    
    /**
     * @dev Convenience function to advance to a specific price point
     * @param _targetPrice The target price in cents (e.g., 75 for $0.75)
     */
    function forceAdvanceToPrice(uint256 _targetPrice) external onlyOwners {
        uint256 targetRound = 0;
        bool foundRound = false;
        
        // Find the round with the target price
        for (uint256 i = 0; i < presaleRoundCount; i++) {
            if (presaleRoundList[i].roundPrice == _targetPrice) {
                targetRound = i;
                foundRound = true;
                break;
            }
        ...
        require(foundRound, "No round found with the target price");
        
        // Use the existing function to advance to that round
        forceAdvanceToRound(targetRound);
    }

    function updateRoundStatus(PresaleRound storage round) internal {
        if (round.remainAmount == 0 || round.isFinished) {
            round.isStarted = false;
            round.isFinished = true;
